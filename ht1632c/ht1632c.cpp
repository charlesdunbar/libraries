...
/* print a string center jusified */

void ht1632c::centertext(int y, char *text, uint8_t color, uint8_t attr, uint8_t bgcolor)
{
    // x_max + 1 since x_max is (32 * num_boards) - 1
	int len = strlen(text), center = (x_max - 2)/2, half_text = len/2, offs;
	uint8_t width = font_width;
	
	for (int i = 0; i < len; i++)
      {
		offs = width * i;
		putchar((center-(half_text*width))+offs, y, text[i], color, attr, bgcolor);
	  }
	sendframe();
}
